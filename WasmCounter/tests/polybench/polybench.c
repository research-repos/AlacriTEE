...
#ifdef POLYBENCH_PRINT_ERR
    if (stat != 0)
      printf ("Error return from gettimeofday: %d", stat);
#endif /* POLYBENCH_PRINT_ERR */
...
void test_fail(char *file, int line, char *call, int retval)
{
  ...
  if (PAPI_is_initialized ())
    PAPI_shutdown ();
  POLYBENCH_DUMP_STRING("Test failed.\n");
  polybench_exit (1);
}
...
#endif /* POLYBENCH_PRINT_ERR */
      POLYBENCH_DUMP_STRING("[ERROR] Inter-array padding requires to use polybench_alloc_data and polybench_free_data.\n");
      polybench_exit (1);
    }
...
xmalloc(size_t alloc_sz)
{
  ...
  sprintf(str_buf, "[PolyBench] xmalloc: allocating memory (alloc_sz=%lu, padded_sz=%lu).\n", alloc_sz, padded_sz);
  POLYBENCH_DUMP_STRING(str_buf);
...
#endif /* POLYBENCH_PRINT_ERR */
      sprintf(str_buf, "[PolyBench] xmalloc: failed to allocate memory (ret=%p, err=%d).\n", ret, err);
      POLYBENCH_DUMP_STRING(str_buf);
      polybench_exit (1);
    }
    sprintf(str_buf, "[PolyBench] xmalloc: allocated memory (ret=%p).\n", ret);
    POLYBENCH_DUMP_STRING(str_buf);
...
{
  char str_buf[1024];
  sprintf(str_buf, "[PolyBench] polybench_free_data: freeing memory (ptr=%p).\n", ptr);
  POLYBENCH_DUMP_STRING(str_buf);
