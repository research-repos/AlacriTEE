...
#  define POLYBENCH_DCE_ONLY_CODE    if (eIdSecSize > 42)
# else
#  define POLYBENCH_DCE_ONLY_CODE
# endif

#ifdef POLYBENCH_DUMP_ENABLED
...
#define POLYBENCH_DUMP_STRING(s) fprintf(POLYBENCH_DUMP_TARGET, "%s", s)
#else
...
#define POLYBENCH_DUMP_STRING(s)
#endif /* POLYBENCH_DUMP_ENABLED */
...
/* Exit syscall */
#define polybench_exit(exitCode) exit(exitCode);

/* WASM support */
# ifdef POLYBENCH_WASM

#  ifdef polybench_exit
#    undef polybench_exit
#  endif /* polybench_exit */
#  define polybench_exit(exitCode) enclave_wasm_exit(exitCode);
extern void enclave_wasm_exit(int32_t);

#  ifdef POLYBENCH_DUMP_STRING
#    undef POLYBENCH_DUMP_STRING
#  endif /* POLYBENCH_DUMP_STRING */
#  define POLYBENCH_DUMP_STRING(s) enclave_wasm_print_string(s);
extern void enclave_wasm_print_string(const char*);

#  ifdef polybench_start_instruments
#    undef polybench_start_instruments
#  endif /* polybench_start_instruments */
#  define polybench_start_instruments enclave_wasm_start_benchmark(); for (int i = 0; i < POLYBENCH_WASM_KERNEL_REPEAT; ++i) {
extern void enclave_wasm_start_benchmark();

#  ifdef polybench_stop_instruments
#    undef polybench_stop_instruments
#  endif /* polybench_stop_instruments */
#  define polybench_stop_instruments } enclave_wasm_stop_benchmark();
extern void enclave_wasm_stop_benchmark();

# endif /* POLYBENCH_WASM */
